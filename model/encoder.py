...
class Encoder(nn.Module):
    ...
    head_num=4, max_dynamic_rg=16, max_static_rg=1400, k=4): # what is k?
        ...
        self.layer_A_I = nn.Sequential(OrderedDict([('layer_A', self.layer_A),
                                                    ('layer_D', self.layer_B),
                                                    ('layer_E', self.layer_C),
                                                    ('layer_F', self.layer_F),
                                                    ('layer_G', self.layer_G),
    ...
    def forward(self, state_feat, agent_batch_mask, road_feat, traffic_light_feat):
        ...
        I_ = self.layer_A_I(state_feat, batch_mask=agent_batch_mask)

        J_ = self.layer_J(I_,C_)
        K_ = self.layer_K(J_,B_)

        L_ = self.layer_L(K_)
        M_ = self.layer_M(L_)

        N_ = self.layer_N(M_,C_)
        O_ = self.layer_O(N_,B_)
...
class SelfAttLayer(nn.Module):
    ...
    def forward(self, x, batch_mask, padding_mask, hidden_mask):
        K = self.layer_K_(x)
        V = self.layer_V_(x)
        Q0 = self.layer_Q0_(x)
        ...
        if self.across_time:
            ...
            energy = torch.matmul(Q,K.T) / self.scale                               # [A,H,T,T]
            ...
            energy = torch.matmul(Q,K.T) / self.scale                               # [T,H,A,A]
...
class CrossAttLayer(nn.Module):
    ...
    def forward(self, agent, rg): # agent -> [A,T,D] / rg -> [G,T,D]
        ...
        energy = torch.matmul(Q,K.T) / self.scale                               # [T,H,A,G]
